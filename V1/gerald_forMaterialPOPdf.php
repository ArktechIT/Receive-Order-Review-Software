<?php
	...
	class materialComputation
	{
		private $batchId;
		private $dataTable = array();
		private $lotNumberArray = array();
		private $customerAliasArray = array();
		...
		{
			$customerAliasArray = array_unique($this->customerAliasArray);
	
			return implode(", ",$customerAliasArray);
		}
		...
		function getPoNumbers()
		{
			...
			$sql = "
				...
				WHERE a.lotNumber IN('".implode("','",$this->lotNumberArray)."')
				GROUP BY b.poNumber
			";
		...
		function loadData()
		{
			...
			$count = 0;
			$sql = "SELECT GROUP_CONCAT(materialComputationId) as materialComputationIds FROM ppic_materialcomputation WHERE batchId = ".$this->batchId." GROUP BY `materialType`, `thickness`, `length`, `width`, `treatment`, `pvc`";
				...
				while($resultMaterialComputationGroup = $queryMaterialComputationGroup->fetch_assoc())
				{
					...
					if($queryMaterialComputation AND $queryMaterialComputation->num_rows > 0)
					{
						...
						while($resultMaterialComputation = $queryMaterialComputation->fetch_assoc())
						{
							...
							$this->customerAliasArray[] = $resultMaterialComputation['customerAlias'];
	...
	$mc = new materialComputation($batchId);
	$dataTableArray = $mc->getData();	
	...
	$pdf->Cell(0,5,$mc->getCustomers(),0,0,'L');
	$pdf->SetFont('Arial', '',9);
	$pdf->Ln();
	
	$pdf->SetFont('Arial', 'B',9);
	$pdf->Cell(20,5,'PO Number : ',0,0,'R');
	$pdf->Cell(0,5,$mc->getPoNumbers(),0,0,'L');
	$pdf->SetFont('Arial', '',9);
	...
	$count = 0;
	foreach($dataTableArray as $value)
	{
