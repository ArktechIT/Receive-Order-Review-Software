	...
	if(isset($_POST['ajaxType']))
	{
		if($_POST['ajaxType']=='updatePattern')
		{
			...
			if(count($lotNoArray))
			{
				...
				{
					$poIdArray = array();
					$sql = "SELECT poId FROM ppic_lotlist WHERE lotNumber IN('".implode("','",$lotNoArray)."') AND identifier = 1";
					$queryLotList = $db->query($sql);
					if($queryLotList AND $queryLotList->num_rows > 0)
					{
						while($resultLotList = $queryLotList->fetch_assoc())
						...
						$sql = "SELECT lotNumber ppic_lotlist WHERE lotNumber IN(".implode(",",$poIdArray).") AND identifier = 1";
						$queryLotList = $db->query($sql);
						if($queryLotList AND $queryLotList->num_rows > 0)
						{
							while($resultLotList = $queryLotList->fetch_assoc())
						...
						$sql = "
							SELECT a.bookingId, GROUP_CONCAT(lotNumber) as lots FROM engineering_bookingdetails as a
							INNER JOIN engineering_booking as b ON b.lotNumber = a.lotNumber
							WHERE a.lotNumber IN('".implode("','",$lotNumberArray)."') AND b.temporaryBookingFlag = 1
						...
						if($queryBookingDetails AND $queryBookingDetails->num_rows > 0)
						{
							while($resultBookingDetails = $queryBookingDetails->fetch_assoc())
							{
								...
								$lots = $resultBookingDetails['lots'];
								
								$sql = "DELETE FROM engineering_bookingdetails WHERE bookingId = ".$bookingId." AND lotNumber IN('".str_replace(",","','",$lots)."')";
								$queryDelete = $db->query($sql);
								
								$sql = "SELECT listId FROM engineering_bookingdetails WHERE bookingId = ".$bookingId."";
								$queryBookingDetailsCount = $db->query($sql);
								if($queryBookingDetailsCount AND $queryBookingDetailsCount->num_rows == 0)
								{
								...
								{
									$totalMaterialRequirement = 0;
									$sql = "SELECT SUM(materialRequirement) as totalMaterialRequirement FROM engineering_bookingdetails WHERE bookingId = ".$bookingId."";
									$queryBookingDetails = $db->query($sql);
									if($queryBookingDetails AND $queryBookingDetails->num_rows > 0)
									{
										$resultBookingDetails = $queryBookingDetails->fetch_assoc();
										$totalMaterialRequirement = $resultBookingDetails['totalMaterialRequirement'];
									}
									
									$totalMaterialRequirementCeil = ceil($totalMaterialRequirement);
									
									$sqlUpdate = "UPDATE engineering_booking SET bookingQuantity = '".$totalMaterialRequirementCeil."' WHERE bookingId = ".$bookingId." LIMIT 1";
				...
				if(count($poIdArray) > 0)
				{
					...
					{
						$sql = "SELECT lotNumber, partId, parentLot, partLevel, workingQuantity, identifier, patternId FROM ppic_lotlist WHERE poId = ".$poId." AND identifier IN(1,2)";
						$queryLotList = $db->query($sql);
						if($queryLotList AND $queryLotList->num_rows > 0)
						{
							while($resultLotList = $queryLotList->fetch_assoc())
								...
								else if($partLevel > 1)
								{
									if($identifier==1)
									{
										if($patternId != -1)
										{
											...
											if($_GET['country']==true)//2021-10-25
											{
							...
							if(count($lotePartsArray)==3)
							{
								if($partLevel==1)
								{
									...
									if($queryWorkSchedule AND $queryWorkSchedule->num_rows > 0)
									{
										...
										$querySystemLotList = $db->query($sql);
										if($querySystemLotList AND $querySystemLotList->num_rows > 0)
										{
											$resultSystemLotList = $querySystemLotList->fetch_assoc();
											//~ $lotNumber = $resultSystemLotList['lotNumber'];
											$dueDate = $resultSystemLotList['recoveryDate'];
