...
$pdf->Cell(20,5,"Customer",'TLR',0,'C');
$pdf->Cell(20,5,"Answer",'TLR',0,'C');
$pdf->Cell(45,5,"Material",'TLR',0,'C');
$pdf->Cell(45,5,"Accessory",'TLR',0,'C');
...
$e=0;
	...
	$lotNumberPO="";
		...
		$answerDate=$reviewdataResult['answerDate'];
		...
		$sql = "SELECT lotNumber FROM ppic_lotlist WHERE poId = ".$poId." and partId = ".$partId." and identifier=1";
		...
		if($customerAliasQuery AND $customerAliasQuery->num_rows > 0)
		{
		...
		$loteItemsArray = $$loteAccessoriesArray = array();
		$sql = "SELECT lotNumber, identifier FROM ppic_lotlist WHERE poId = ".$poId."";
		$queryLotList = $db->query($sql);
		if($queryLotList AND $queryLotList->num_rows > 0)
		{
			while($resultLotList = $queryLotList->fetch_assoc())
			{
				...
				{
					$loteAccessoriesArray[] = $lote;
				}
			}
		}
		
		$materialUsed = "";
		if(count($loteItemsArray) > 0)
		{
			$bookingIdArray = array();
			$sql = "SELECT DISTINCT bookingId FROM engineering_bookingdetails WHERE lotNumber IN('".implode("','",$loteItemsArray)."')";
			...
			$sql = "SELECT inventoryId FROM engineering_booking WHERE bookingId IN(".implode(",",$bookingIdArray).")";
			...
			$materialUsed = implode(", ",$inventoryIdArray);
		}
		
		$accesoriesUsed = "";
		if(count($loteAccessoriesArray) > 0)
		{
			$inventoryIdArray = array();
			$sql = "SELECT DISTINCT inventoryId FROM system_accessorybooking WHERE lotNumber IN('".implode("','",$loteAccessoriesArray)."')";
			$queryAccessoryBooking = $db->query($sql);
			if($queryAccessoryBooking AND $queryAccessoryBooking->num_rows > 0)
			{
				while($resultAccessoryBooking = $queryAccessoryBooking->fetch_assoc())
				{
					$inventoryIdArray[] = $resultAccessoryBooking['inventoryId'];
				}
			}
			$accesoriesUsed = implode(", ",$inventoryIdArray);
		}
		$initialX = $pdf->GetX();
		$pdf->SetFont('Arial','',8);
		$pdf->Cell(5,5,($e+1),0,0,'C');
		$pdf->Cell(20,5,$customerName,0,0,'L');
		$pdf->Cell(20,5,$poNumber,0,0,'L');
		$pdf->Cell(20,5,$lotNumberPO,0,0,'L');
		$pdf->Cell(35,5,$partNumber,0,0,'L');
		
		$pdf->SetFont('Arial','B',10);
		$pdf->Cell(15,5,$poQuantity,0,0,'L');
		$pdf->Cell(20,5,$deliveryDate,0,0,'C');
		$pdf->Cell(20,5,$answerDate,0,0,'C');
		$pdf->SetFont('Arial','',8);
		$currentX = $pdf->GetX();
		$currentY = $pdf->GetY();
		$pdf->MultiCell(45,5,$materialUsed,0);
		$currentY1 = $pdf->GetY();
		$heightTemp = $currentY1 - $currentY;
		$pdf->SetXY(($currentX+45),$currentY);
		$currentX = $pdf->GetX();
		$currentY = $pdf->GetY();
		$pdf->MultiCell(45,5,$accesoriesUsed,0);
		$currentY1 = $pdf->GetY();
		$heightTemp1 = $currentY1 - $currentY;
		$height = ($heightTemp1 > $heightTemp) ? $heightTemp1 : $heightTemp;
		$pdf->SetXY(($currentX+45),$currentY);
		$pdf->Cell(40,5,"",0,0,'C');
		/*
		...
		*/
		...
		$pdf->Cell(20,$height,'',1);
		$pdf->Cell(20,$height,'',1);
		$pdf->Cell(35,$height,'',1);
		$pdf->Cell(15,$height,'',1);
		$pdf->Cell(20,$height,'',1);
		$pdf->Cell(20,$height,'',1);
		$pdf->Cell(45,$height,'',1);
		$pdf->Cell(45,$height,'',1);
		$pdf->Cell(40,$height,'',1);
		
		
		$pdf->SetFont('Arial','',8);
		$pdf->Ln();
		$pdf->SetY(($currentY+$height));
		$e++;
	}
