<?php
	...
	if(isset($_POST['submit']))
	{
		...
		$sql = "SELECT `materialComputationId`, `customerAlias`, `materialType`, `thickness`, `length`, `width`, `treatment`, `pvc`, `quantity`, `finalQuantity`, `status`, `idnumber`, `inputDateTime`, `lotNumber`, `blankingProcess`, `alternateMaterial`, `productId`, `splitQtyArrival` FROM ppic_materialcomputation WHERE lotNumber ='' AND status = 1 AND productId > 0";
		$queryMaterialComputation = $db->query($sql);
		if($queryMaterialComputation AND $queryMaterialComputation->num_rows > 0)
		{
							...
							if($queryInsert)
							{
								$sql = "
									INSERT INTO `ppic_materialcomputation`
										(	`customerAlias`, `materialType`, `thickness`, `length`, `width`, `treatment`, `pvc`, `quantity`, 	`finalQuantity`,`status`, `idnumber`, `inputDateTime`,	`lotNumber`,		`dateNeeded`,		`blankingProcess`, `alternateMaterial`, `productId`, `materialComputationIdSource`, `batchId`)
									SELECT	`customerAlias`, `materialType`, `thickness`, `length`, `width`, `treatment`, `pvc`, '".$forPo."',	'".$forPo."', 	`status`, `idnumber`, NOW(), 			'".$lotNumber."',	'".$dateNeeded."',	`blankingProcess`, `alternateMaterial`, `productId`, '".$materialComputationId."',	'".$batchId."'
	...
	$sql = "SELECT `materialComputationId`, `customerAlias`, `materialType`, `thickness`, `length`, `width`, `treatment`, `pvc`, `quantity`, `finalQuantity`, `status`, `idnumber`, `inputDateTime`, `lotNumber`, `blankingProcess`, `alternateMaterial`, `productId`, `splitQtyArrival` FROM ppic_materialcomputation WHERE lotNumber ='' AND status = 1";
	...
	if($queryMaterialComputation AND $queryMaterialComputation->num_rows > 0)
	{
		while($resultMaterialComputation = $queryMaterialComputation->fetch_assoc())
		{
			...
			$noData = 0;
			$arrayOuput = $listIdArray = array();
			$sql = "SELECT materialTreatmentId, materialId, treatmentId, pvc FROM purchasing_materialtreatment WHERE materialId IN(".implode(",",$materialIdArray).") AND treatmentId = ".$cadcamTreatmentId."";
			$queryMaterialTreatment = $db->query($sql);
			if($queryMaterialTreatment AND $queryMaterialTreatment->num_rows > 0)
			{
				while($resultMaterialTreatment = $queryMaterialTreatment->fetch_assoc())
					...
					if($queryMaterial->num_rows > 0)
					{
						...
						if($querySupplies AND $querySupplies->num_rows > 0)
						{
							while($resultSupplies = $querySupplies->fetch_assoc())
							{
								...
								$arrayOuput[$listId] = $materialType." t".$resultMaterial['thickness']." ".$resultMaterial['length']." x ".$resultMaterial['width']." ".$treatmentName.$pvc." (".$supplierAlias.")";
							}
						}
						else
						{
							$arrayOuput['noData'.(++$noData)] = $materialType." t".$resultMaterial['thickness']." ".$resultMaterial['length']." x ".$resultMaterial['width']." ".$treatmentName.$pvc." (NO PRICE DATA)";
						}
					}
				}
			}
			
			if($productId==0 AND count($listIdArray) == 1)
			{
				$productId = $listIdArray[0];
				$sql = "UPDATE ppic_materialcomputation SET productId = ".$productId." WHERE materialComputationId = ".$materialComputationId." LIMIT 1";
				$queryUpdate = $db->query($sql);
			}
