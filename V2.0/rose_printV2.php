...
$e=0;
	...
	$queryRoReviewData = $db->query($sql);
	if($queryRoReviewData AND $queryRoReviewData->num_rows > 0)
	{
		while($resultRoReviewData = $queryRoReviewData->fetch_assoc())
		{
			$poIdArray[] = $resultRoReviewData['poId'];
		}
	}
	
	$lotNumberArray = array();
	$sql = "SELECT lotNumber FROM ppic_lotlist WHERE poId IN('".implode(",",$poIdArray)."') AND identifier = 1";
	...
	$sectionDataArray['Engr'][] = array(312,430,431,432);
	$sectionDataArray['Engr'][] = array();
	$sectionDataArray['Prod'][] = array();
	$sectionDataArray['Prod'][] = array(3,42,1,2,6);
	$sectionDataArray['Insp1'][] = array(92,165,168,238,239,89,169,170,223,240,140);
	$sectionDataArray['Insp1'][] = array();
	$sectionDataArray['WH1'][] = array(223,280,521);
	$sectionDataArray['WH1'][] = array();
	$sectionDataArray['Subcon'][] = array(145,172,228,137);
	$sectionDataArray['Subcon'][] = array();
	$sectionDataArray['Prod2'][] = array();
	$sectionDataArray['Prod2'][] = array(12,48,6);
	$sectionDataArray['Final Insp'][] = array(187,224,278,279,253,94,358);
	$sectionDataArray['Final Insp'][] = array();
	$sectionDataArray['WH2'][] = array(187,224,278,279,253,94,358);
	$sectionDataArray['WH2'][] = array();
	$sectionDataArray['Due Date'][] = array(518);
	$sectionDataArray['Due Date'][] = array();
	
	$pdf->Ln();
	$pdf->SetFont('Arial','B',10);
	$pdf->Cell(30,5,'',1,0,'C');
	$pdf->Cell(30,5,'Start',1,0,'C');
	$pdf->Cell(30,5,'End',1,0,'C');
	$pdf->Cell(30,5,'Remarks',1,0,'C');
	$pdf->Cell(30,5,'Signature',1,1,'C');	
	
	$targetTarget = '0000-00-00';
	$pdf->SetFont('Arial','',10);
	foreach($sectionDataArray as $key => $values)
	{
		$processArray = $values[0];
		$sectionArray = $values[1];
		
		if(count($processArray) > 0)
		{
			$filterSql = "processCode IN(".implode(",",$processArray).")";
		}
		else if(count($sectionArray) > 0)
		{
			$filterSql = "processSection IN(".implode(",",$sectionArray).")";
		}
		
		$targetStart = 'N/A';
		$sql = "SELECT targetStart FROM ppic_workschedule WHERE lotNumber IN('".implode("','",$lotNumberArray)."') AND ".$filterSql." AND targetStart >= '".$targetTarget."' ORDER BY targetStart LIMIT 1";
		//~ $sql = "SELECT targetFinish FROM ppic_workschedule WHERE lotNumber IN('".implode("','",$lotNumberArray)."') AND ".$filterSql." ORDER BY targetFinish LIMIT 1";
		...
		if($queryWorkschedule AND $queryWorkschedule->num_rows > 0)
		{
			...
			//~ $targetStart = $resultWorkschedule['targetFinish'];
		}
		
		$targetFinish = 'N/A';
		$sql = "SELECT targetFinish FROM ppic_workschedule WHERE lotNumber IN('".implode("','",$lotNumberArray)."') AND ".$filterSql." AND targetStart >= '".$targetTarget."' ORDER BY targetFinish DESC LIMIT 1";
		$queryWorkschedule = $db->query($sql);
		if($queryWorkschedule AND $queryWorkschedule->num_rows > 0)
		{
			$resultWorkschedule = $queryWorkschedule->fetch_assoc();
		...
		if($targetFinish!='N/A')
		{
			$targetTarget = $targetFinish;
		}
		
		$pdf->Cell(30,5,$key,1,0,'C');
		$pdf->Cell(30,5,$targetStart,1,0,'C');
		$pdf->Cell(30,5,$targetFinish,1,0,'C');
		$pdf->Cell(30,5,'',1,0,'C');
		$pdf->Cell(30,5,'',1,1,'C');		
	}
	...
	/*
	$sectionDataArray = array();
	$sectionDataArray[] = 'Engr';
	$sectionDataArray[] = 'Prod';
	$sectionDataArray[] = 'Insp1';
	$sectionDataArray[] = 'WH1';
	$sectionDataArray[] = 'Subcon';
	$sectionDataArray[] = 'Prod2';
	$sectionDataArray[] = 'Final Insp';
	$sectionDataArray[] = 'WH2';
	$sectionDataArray[] = 'Due Date';
	...
	$pdf->SetFont('Arial','',10);
	foreach($sectionDataArray as $key)
	{
		if($key == 'Engr')
		{
			$sqlStart = "SELECT targetStart FROM ppic_workschedule WHERE lotNumber IN('".implode("','",$lotNumberArray)."') AND processCode IN(312, 430, 431, 432) ORDER BY targetStart LIMIT 1";
			$sqlEnd = "SELECT targetFinish FROM ppic_workschedule WHERE lotNumber IN('".implode("','",$lotNumberArray)."') AND processCode IN(312, 430, 431, 432) ORDER BY targetFinish DESC LIMIT 1";
		}
		else if($key == 'Prod')
		{
			$sqlStart = "SELECT targetStart FROM ppic_workschedule WHERE lotNumber IN('".implode("','",$lotNumberArray)."') AND processSection IN(3,42,1) ORDER BY targetStart LIMIT 1";
			$sqlEnd = "SELECT targetFinish FROM ppic_workschedule WHERE lotNumber IN('".implode("','",$lotNumberArray)."') AND processCode IN(312, 430, 431, 432) ORDER BY targetFinish DESC LIMIT 1";
		}
		
		
		$queryWorkschedule = $db->query($sqlStart);
		if($queryWorkschedule AND $queryWorkschedule->num_rows > 0)
		{
			$resultWorkschedule = $queryWorkschedule->fetch_assoc();
			$targetStart = $resultWorkschedule['targetStart'];
		...
		$queryWorkschedule = $db->query($sqlEnd);
		if($queryWorkschedule AND $queryWorkschedule->num_rows > 0)
		{
			$resultWorkschedule = $queryWorkschedule->fetch_assoc();
			$targetFinish = $resultWorkschedule['targetFinish'];
