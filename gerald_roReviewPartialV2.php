<?php
	...
	if(isset($_POST['partialData']))
	{
		...
		$partialDataArray = explode("\n",$partialData);
		
		if(count($partialDataArray) > 0)
		{
			$prevLotNumber = "";
			foreach($partialDataArray as $value)
			{
				$valueArray = explode("|",$value);
				$lotNumber = $valueArray[0];
				$quantity = $valueArray[1];
				
				if($prevLotNumber!=$lotNumber)
				{
					$i = 0;
	...
	if(isset($_POST['submit']))
	{
		...
		$lotNumber = '';
		$workingQuantity = 0;
		$sql = "SELECT lotNumber, workingQuantity, partId, identifier, partLevel, parentLot FROM ppic_lotlist WHERE poId = ".$poId." AND lotNumber IN('".implode("','",$lotNumberArray)."') AND identifier IN(1,2) ORDER BY FIELD(lotNumber,'".implode("','",$lotNumberArray)."')";
		$queryLotList = $db->query($sql);
		if($queryLotList AND $queryLotList->num_rows > 0)
		{
			while($resultLotList = $queryLotList->fetch_assoc())
			{
				...
				echo "<br>".$indent."<span style='text-decoration:underline;'>".$partNumber."</span>";
				...
				while($i < $lotCount)
				{
					...
					$lotPartial = $lotNumber.$dashNumber;
					...
					$partialQuantity = $quantity * $multiplier;
					
					echo "<br>&nbsp;&nbsp;&nbsp;&nbsp;".$indent.$lotPartial.$spacer." &nbsp; "." <input type='number' name='lotQtyArray[]' style='background-color:yellow;' class='inputQuantity ".$lotNumber." ".$batchNo."' data-lot-number='".$lotNumber."' data-batch-no='".$batchNo."' data-multiplier='".$multiplier."' value='".$partialQuantity."' min='0' max='".$workingQuantity."' form='partialFormId'>";
				}
			}
			
			echo "<form action='' method='post' id='submitFormId'></form>";
			echo "<button id='submitId'>Submit</button>";
			echo "<input type='hidden' name='poId' value='".$poId."' form='submitFormId'>";
			echo "<input type='hidden' name='dueDate' value='".$dueDate."' form='submitFormId'>";
			echo "<textarea id='textArea' name='partialData' style='visibility:hidden;height:0.1px;' form='submitFormId'></textarea>";
		}
		
		?>
		<script src='/<?php echo v;?>/Common Data/Templates/jquery.js'></script>
		<script>
			function autoChangeQuantity(thisObj)
			{
				var thisVal = parseFloat(thisObj.val());
				var maxQty = parseFloat(thisObj.attr('max'));
				var lotNumber = thisObj.data('lotNumber');
				var batchNo = thisObj.data('batchNo');
				var thisIndex = parseFloat(thisObj.index('.'+lotNumber));
				var limitCount = parseFloat(($('.'+lotNumber).length));
				var totalQty = 0;
				$('.'+lotNumber).each(function(i){
				...
				if(totalQty != maxQty)
				{
					...
					{
						i--;
						var lastThis = $("."+lotNumber+":eq("+i+")");
						var lastThisVal = parseFloat(lastThis.val());
						var lastBatchNo = lastThis.data('batchNo');
						console.log(lastThisVal);
						if(totalQty > maxQty)
						{
							var diff = totalQty - maxQty;
							var lastThisNewVal = lastThisVal - diff;
						}
						else
						{
							var diff = maxQty - totalQty;
							var lastThisNewVal = lastThisVal + diff;
						}
						...
						//~ else
						//~ {
							lastThis.val(lastThisNewVal);
						//~ }
						
						//~ $("."+lastBatchNo).val(0);			
						
						//~ nextThisNewVal = nextThisVal + nextThisNewVal;
						//~ console.log(nextThisNewVal);
						//~ alert('asd');
					}					
					while(i > 0 && lastThisNewVal < 0)
					*/
					
					
					var eq = ((thisIndex+1) == limitCount) ? 0 : (thisIndex+1);
					
					var nextThis = $("."+lotNumber+":eq("+eq+")");
					var nextThisVal = parseFloat(nextThis.val());
					var nextBatchNo = nextThis.data('batchNo');					
					
					if(totalQty > maxQty)
					{
						var diff = totalQty - maxQty;
						var nextThisNewVal = nextThisVal - diff;
					}
					else
					{
						var diff = maxQty - totalQty;
					...
					var i = 0;
					while(nextThisNewVal < 0 && i < limitCount)
					{
						++i;
						var lastThis = $("."+lotNumber+":eq("+(limitCount-i)+")");
						var lastThisVal = parseFloat(lastThis.val());
						var lastBatchNo = lastThis.data('batchNo');
					...
					nextThis.val(nextThisNewVal);
					
					$("."+nextBatchNo).each(function(i){
						var multiplier = parseFloat($(this).data('multiplier'));
						$(this).val(nextThisNewVal*multiplier);
					});
				}
			...
			$(function(){
				...
				$("#submitId").click(function(){
					...
					$(".inputQuantity").each(function(i){
						...
						partialArray[i] = lotNumber+"|"+quantity;
					});
					
					$('#textArea').val(partialArray.join("\n"));
					$("#submitFormId").submit();
				});
